...
│   ├── shop.css                  Styles Boutiques (préfixe `.sx-`) : sommaire par famille, cartes+vignettes, badges de statut, en-tête de page boutique, podium, **tableau `.sx-table` + mode édition** ; plus `.sc-item-img` et `.shop-toolbar`, et la valorisation d'événement (préfixe `.sxe-`, cf. §5)
...
│   ├── shop-event.js             Valorisation d'un événement (section `#sp-event`) : grille des achats par jour, gains cumulés, % récupéré. Chargé UNIQUEMENT par les pages ayant un `data/events/<slug>.json` (cf. §5)
...
| `shop/<boutique>.html` | Une boutique : en-tête, podium, tableau (1 ligne/objet) + mode édition. **Deux lectures** au choix (pastilles 💎 Gemmes / € Euros, cf. §6) | `shop-core.js`, `shop-page.js` (+ `shop-event.js` si la boutique a un fichier d'événement) | `style.css`, `db.css`, `shop.css` | idem + `shopcalc_euro.json` (via `window.SHOP_SLUG`) |
...
- **Google Analytics (GA4, `G-M1QG4XSD3K`)** : extrait officiel `gtag.js` collé **verbatim** dans le `<head>` de chaque page, juste après le script de thème (qui doit rester premier). Présent sur les 66 pages HTML, `404.html` comprise ; **seule exception `google55512fe842dbeeaf.html`**, le jeton de vérification Search Console. L'identifiant est donc dupliqué 66 fois : le changer = un remplacement global. Vérifié : le site se rend normalement quand le script est bloqué (bloqueur de pub).
