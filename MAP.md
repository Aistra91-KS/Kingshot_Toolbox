...
- **Piège de la sidebar sticky — rien ne doit chevaucher le pied de page** : `.sidebar` est `position:sticky` et se confine au **body**, pas à la ligne flex ; en bas de page elle descend donc *dans* le pied de page. Deux fausses pistes écartées : lui donner un `z-index` (la bande passe devant la sidebar et **bloque le bouton « Sauvegarde Globale »**), ou ne rien faire (la sidebar passe devant la bande et la tronque — refusé par Paul : le pied de page doit être **sous chaque élément**, quitte à être plus bas). Solution retenue : `ftrTrackSidebar()` (js/footer.js) publie dans **`--footer-overlap`** la hauteur du pied de page déjà visible (mesure au `scroll`/`resize` + `ResizeObserver`, throttlée en `requestAnimationFrame`), et la `max-height` de `.sidebar` (§4) la retranche. La sidebar se raccourcit donc à l'approche du pied de page et s'arrête pile au-dessus ; son défilement interne garde ses commandes accessibles. La variable vaut `0px` par défaut : pages sans sidebar, sans pied de page et mobile (< 768px, où `.sidebar` repasse en `static` et `max-height:none`) ne sont pas concernées. Le pied de page reste **volontairement non positionné** — ne pas lui remettre de z-index.
