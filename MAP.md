...
| `shop/items-euro.html` | Prix réel des objets : prix unitaire en € ou $ + pack d'origine. **Lecture seule** (donnée admin), pastilles € / $, recherche par objet **ou par pack**. Ne liste que les objets qu'un pack chiffre (43 / 87) ; la note de bas de tableau annonce cette couverture | `shop-core.js`, `shop-items-euro.js` | `style.css`, `db.css`, `shop.css` | `shopcalc_items.json`, `shopcalc_euro.json` |
