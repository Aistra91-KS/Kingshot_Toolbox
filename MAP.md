...
- **Piège de la sidebar sticky** : `.sidebar` est `position:sticky` et se confine au **body**, pas à la ligne flex — elle descend donc jusqu'au pied de page. Une version antérieure donnait un `z-index` à la bande : elle passait devant la sidebar et **rendait le bouton « Sauvegarde Globale » impossible à cliquer** en bas de page. Le pied de page n'est donc **volontairement pas positionné** — la sidebar, elle, l'est, et passe devant lui —, et c'est le **contenu** qui s'écarte : classe `sf-with-sidebar` (posée par `footer.js` si `body > .sidebar`) → `.sf-inner { padding-left: 360px }` (sidebar 340 + gouttière 20), au-dessus de 768px seulement (sous ce seuil le body s'empile en colonne). Ne pas « corriger » cette absence de z-index.
