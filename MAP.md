...
- **Un seul favicon par *hostname*** : Google le prend sur la page d'accueil du domaine, ici `https://aistra91-ks.github.io/` — qui n'appartient pas à ce dépôt (même limite que `robots.txt` ci-dessus). Les balises en dur sont donc nécessaires mais **pas suffisantes** : il faut aussi un dépôt `aistra91-ks.github.io` (site utilisateur) qui déclare la même icône à la racine, ou un nom de domaine propre.
