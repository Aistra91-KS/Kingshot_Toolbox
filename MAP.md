...
│   ├── truegold_war_advanced_db.json  Académie de Guerre « Advanced » : 92 techs, totaux par tech (valeurs arrondies) — **non branché** à un outil
...
| `truegold_war_advanced_db.json` | **Objet** `{meta, categories, pageTotals, techs}`. 92 techs de l'arbre **« War Academy Advanced »** (1010 niveaux), 4 catégories (`special` 9 / `economy` 2 / `capacity` 7 / `combat` 74). Chaque tech : `{id, baseId, tier, tierRoman, name{EN,FR}, category, effect{EN,FR}, iconSlug, maxLevel, totals{ttg,dust,gold,bread,wood,stone,iron,timeMinutes,power}, totalsRaw}`. ⚠️ **`totals` = coût TOTAL de la tech (niv. 0 → maxLevel), pas un coût par niveau** — contrairement à `truegold_war_db.json`. ⚠️ **Valeurs arrondies** : la source affichait `1.1K`/`12.1M`, `totalsRaw` conserve la chaîne d'origine pour ré-extraction. Manquent : coûts par niveau, prérequis, `reqWA`, valeurs de buff, noms FR. **FR volontairement à `null`** : la mise à jour n'est pas encore sortie sur le serveur de Paul (estimation ~50 j au 2026-08-09), les libellés officiels n'existent donc nulle part — **ne pas les inventer ni les traduire automatiquement**, attendre de pouvoir les lire en jeu. `meta.crossCheck` compare la somme des lignes au total de la page — colonne `ttg` en `MISMATCH` assumé (cf. §6 *Provenance*). **Provenance** : extrait d'un instantané HTML de `kingshotoptimizer.com/data/advanced-truegold-research` (v1.13.0), script d'extraction non commité. |
